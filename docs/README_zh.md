...
│   │   ├── ipc.rs                 # 27 个 #[tauri::command] API 接口
│   │   ├── state.rs               # AppState 状态及内置预设初始化
...
- 水印功能依赖系统字体（macOS 将自动从 `/System/Library/Fonts/Helvetica.ttc` 加载，找不到则跳过）
...
- ⚡️ **极致的 rayon 并行处理**：不仅采用了单图多线程色彩流水线，更实现了批量导出的双层级并行优化，榨干多核 CPU 性能。
