...
const POSITIONS: { value: Watermark["position"]; label: string }[] = [
...
export function ExportDialog({ open, onOpenChange }: ExportDialogProps) {
  ...
  const [wmText, setWmText] = useState("FujiSim");
  const [wmPos, setWmPos] = useState<Watermark["position"]>("bottom_right");
    ...
    const resize: ResizeSpec | null =
      resizeMode === "long_edge"
        ...
        : resizeMode === "percent"
          ? { percent }
          : null;
    const watermark: Watermark | null = wmEnabled
      ? { text: wmText, position: wmPos, opacity: wmOpacity, size: wmSize }
      : null;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <DialogDescription>使用当前胶片参数生成新文件，不会修改原文件。</DialogDescription>
              ...
              {destKind === "subfolder" ? (
                ...
                <div className="flex flex-1 gap-1">
                  <Input
                    ...
                    placeholder="/path"
                  />
                  <Button size="sm" variant="secondary" onClick={pickDestPath}>选...</Button>
                </div>
              )}
            </div>
          </Row>
            ...
            {wmEnabled && (
              <div className="mt-2 space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <Select
                    value={wmPos}
                    onValueChange={(v) => setWmPos(v as Watermark["position"])}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
